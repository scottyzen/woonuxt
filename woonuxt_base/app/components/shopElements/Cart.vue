...
const cartContents = computed(() => cart.value ?? { isEmpty: true, contents: { nodes: [], productCount: 0 } });
</script>
...
<template>
  <div class="fixed top-0 bottom-0 right-0 z-50 flex flex-col w-11/12 max-w-lg overflow-x-hidden bg-white shadow-lg">
    ...
    <EmptyCart v-if="cartContents && !cartContents.isEmpty" class="rounded-lg shadow-lg p-1.5 hover:bg-red-400 hover:text-white" />

    <div class="mt-8 text-center">
      {{ $t('messages.shop.cart') }}
      <span v-if="cartContents?.contents?.productCount"> ({{ cartContents.contents.productCount }}) </span>
    </div>

    <ClientOnly>
      <template v-if="cartContents && !cartContents.isEmpty">
        <ul class="flex flex-col flex-1 gap-4 p-6 overflow-y-scroll md:p-8">
          <CartCard v-for="item in cartContents.contents?.nodes" :key="item.key" :item="item" />
        </ul>
        <div class="px-8 mb-8">
          <NuxtLink
      ...
      <!-- Empty Cart Message -->
      <EmptyCartMessage v-else-if="cartContents && cartContents.isEmpty" />
      <!-- Cart Loading -->
