...
<template>
  <ul v-if="topMenu?.length" class="flex items-center gap-6">
    <li
      ...
      @mouseleave="onLeave"
    >
      <NuxtLink :to="item.uri" class="py-2 px-2 text-gray-700 hover:text-black transition">
        {{ item.label }}
      </NuxtLink>

      <!-- ==== MEGA PANEL (FULL-WIDTH, NO RADIUS) ==== -->
      <transition name="mega-panel">
        <div
          v-if="openIndex === i && item.columns.length"
          class="fixed inset-x-0 z-50 bg-white border-t border-gray-100 shadow-[0_24px_64px_-24px_rgba(0,0,0,.18)] rounded-none"
          style="top:56px"                             <!-- onder header (h-14 = 56px) -->
          @mouseenter="onEnter(i)"                    <!-- houd open bij hover op panel -->
          @mouseleave="onLeave"
        >
          <div class="container px-4 py-6">
            <div class="grid grid-cols-4 gap-8">
              <div v-for="col in item.columns" :key="col.title" class="min-w-[180px]">
                <!-- lvl-1 titel (donker + klikbaar) -->
                <NuxtLink :to="col.uri" class="block font-semibold text-gray-900 hover:text-black mb-2">
                  {{ col.title }}
                </NuxtLink>
                ...
                <ul class="space-y-1">
                  <li v-for="sub in col.items" :key="sub.uri">
                    <NuxtLink :to="sub.uri" class="text-sm text-gray-900 hover:text-black">
                      {{ sub.label }}
                    </NuxtLink>
                  </li>
                </ul>
