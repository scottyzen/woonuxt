<template>
	<div>
		<NuxtLink class="flex relative items-center justify-center" to="/products">
			<NuxtImg width="1400" height="797" class="object-cover w-full lg:h-[580px]" src="/images/hero.jpeg" alt="Hero image" loading="eager" format="webp" fit="outside" preload />
...
export default {
	...
	async asyncData({ $graphql, params }) {
		...
		const { products: bestSellers } = await $graphql.default.request(
			GET_PRODUCTS,
			{ first: 8, orderby: [{ field: 'TOTAL_SALES', order: 'DESC' }] }
		);
		const { products: latesProducts } = await $graphql.default.request(
			GET_PRODUCTS,
			{ first: 8, orderby: [{ field: 'DATE', order: 'DESC' }] }
		);
