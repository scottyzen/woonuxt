<template>
  <div class="-mx-4 md:m-0 md:w-[500px]" v-if="mainImage">
    ...
    <NuxtImg class="object-contain rounded-2xl w-full min-w-[350px]" width="700" height="700" format="webp" fit="outside" v-for="(node, i) in gallery.nodes" :key="i" :src="node.sourceUrl" v-show="imageToShow === i + 1" placeholder />
    <div v-if="gallery.nodes.length" class="my-4 gallery-images">
      <NuxtImg class="cursor-pointer rounded-2xl" width="110" height="140" format="webp" :src="firstImage" @click.native="changeImage(null)" placeholder />
      <NuxtImg class="cursor-pointer rounded-2xl" width="110" height="140" fit="outside" v-for="(node, i) in gallery.nodes" :key="i" :src="node.sourceUrl" @click.native="changeImage(i + 1)" placeholder />
    </div>
  </div>
</template>
