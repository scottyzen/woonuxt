<template>
  <li class="flex gap-4 items-center relative" v-if="item">
    <NuxtImg class="rounded-xl h-16 w-16" v-if="productType.image" :src="productType.image.sourceUrl" :alt="productType.image.altText || productType.name" :title="productType.image.altText || productType.name" />
    <div class="flex-1">
    ...
    <div class="bg-white flex bg-opacity-50 inset-0 absolute items-center justify-center" v-if="isLoading">
...
export default {
  ...
  methods: {
    async updateQuantity(quantity) {
      try {
        this.isLoading = true;
        const { updateItemQuantities } = await this.$graphql.default.request(updateCartQuantity, { key: this.item.key, quantity: parseInt(quantity) });
