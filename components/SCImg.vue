...
export default {
	...
	head() {
		return {
			link: this.preload
				? [{ rel: 'preload', href: this.sourceUrl, as: 'image' }]
				: [],
		};
	},
	computed: {
		base() {
			...
			return process.env.NODE_ENV === 'production'
				? `https://images.weserv.nl/?url=https://${this.$config.domain}/${this.src}`
				: this.src;
		},
		sourceUrl() {
			if (this.sm) {
		...
		srcset() {
			...
			const srsArray = sizes
				.slice(0, sizesBelowMaxWidth.length + 1)
				.map((size) => {
					const ratio = this.width / this.height;
					const width =
						parseInt(size) > parseInt(this.width) ? this.width : size;
