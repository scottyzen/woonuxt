...
export default defineComponent({
  setup(props, context) {
    ...
    const { $graphql, store } = useContext()
    const isAlive = ref(true);

    const removeItemFromCart = async () => {
      const key = context.attrs.item.key;
      const quantity = 0;
      const { updateItemQuantities } = await $graphql.default.request(UPDATE_CART_QUANTITY, { key, quantity });
      store.commit("updateCart", updateItemQuantities.cart);
    };
