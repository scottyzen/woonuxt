<template>
	<li class="flex py-4 gap-4 items-center">
		...
		<NuxtLink :to="`/product/${product.slug}`">
			<Nuxtimg class="rounded-xl h-12 w-12 md:h-20 md:w-20" v-if="product.image" :src="product.image.sourceUrl" :alt="product.image.altText || product.name" :title="product.image.altText || product.name" />
		</NuxtLink>
