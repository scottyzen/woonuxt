<template>
  <div class="mx-auto max-w-lg my-16 min-h-96 lg:my-24">
    <h1 class="font-bold text-xl mb-4 lg:text-3xl">{{ $t('messages.general.myAccount') }}</h1>
    <form class="mb-4" @submit.prevent="loginOrRegister(userInfo)">
      ...
      <label for="username"
        >{{ $t('messages.general.username') }} <span class="text-red-500">*</span> <br />
        <input id="username" v-model="userInfo.username" placeholder="Username" type="text" required />
      </label>
      <label for="password"
        >{{ $t('messages.general.password') }} <span class="text-red-500">*</span> <br />
      ...
      <button class="flex mt-4 text-lg gap-4 justify-center items-center">
        ...
        <span>{{ formView == 'login' ? $t('messages.general.login') : $t('messages.general.register') }}</span>
      </button>
    </form>
    <div v-if="formView == 'login'" class="my-4 text-center">
      {{ $t('messages.general.noAccount') }}
      <a class="cursor-pointer underline" @click="formView = 'register'">{{ $t('messages.general.please') }} {{ $t('messages.general.accountRegister') }}</a>.
    </div>
    <div v-if="formView == 'register'" class="my-4 text-center">
      {{ $t('messages.general.hasAccount') }}
      <a class="cursor-pointer underline" @click="formView = 'login'">{{ $t('messages.general.please') }} {{ $t('messages.general.accountLogin') }}</a>.
    </div>
  </div>
</template>
...
const login = async (userInfo) => {
  ...
  if (error && error === 'invalid_username') {
    ...
    errorMessage.value = '';
    message.value = $t('messages.general.loggingIn');
  }
};
...
const loginOrRegister = async (userInfo) => {
  if (formView.value === 'register') {
    ...
    if (success) {
      errorMessage.value = '';
      message.value = $t('messages.general.accountCreated') + ' ' + $t('messages.general.loggingIn');
      setTimeout(() => {
        login(userInfo);
      }, 2000);
    } else {
